...
if (($config = @file_get_contents($config_file_name))) {
    $config_contents = json_decode($config, true);
    if ($config_contents['cache_lifetime'] > 0) {
        $cache_lifetime = $config_contents['cache_lifetime'];
    }
    if (preg_match('#\w+\.json#', $config_contents['cache_file_name'])) {
        $cache_file_name = $config_contents['cache_file_name'];
    }
}

if (($cache_contents = @file_get_contents($cache_file_name))) {
    $cache_contents = json_decode($cache_contents, true);
    if (($cache_contents['timestamp']  + $cache_lifetime) > $current_timestamp && $cache_contents['events']) {
...
// $crawler->filter('li[data-testid="tsguide-itm"]')->each(function (Crawler $node, $i) use (&$events) {
// echo "<pre>".$i."</pre>";
// });
// exit();
...
$found_nodes = $crawler->filter('li[data-testid="tsguide-itm"]');
if ($found_nodes->count() < 1) {
    header($_SERVER['SERVER_PROTOCOL'] . ' 500 Internal Server Error: scraper error', true, 500);
    serve_result($events);
    exit;
}

$found_nodes->each(function (Crawler $node) use (&$events) {
    ...
    function setEvent(&$node, $is_fictive = false) {
        ...
        $hour = trim($node->filter('p:nth-child(3)')->text());
        $title = trim($node->filter('h3[data-testid="ts-tsTitle"]')->text());
        try {
            $subtitle = trim($node->filter('p[data-testid="ts-tsSubtitle"]')->text());
        }
        catch (Exception $error) {
            $subtitle = '';
        }
        $thumbnail = trim($node->filter('img[data-testid="ts-tsImg"]')->attr('src'));
        // $thubnail_url = $thumbnail;
        $thubnail_url = str_replace('160x90', '265x149', $thumbnail);
